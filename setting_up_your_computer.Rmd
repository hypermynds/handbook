...
2. when the setup assistant asks you to enrol or activate the company profile,
   sign in with your company email address and the same password used for your
   Google Workspace account;
3. complete the setup assistant without disconnecting;
4. create your local user if prompted;
5. if Apple sign-in is requested, use your managed company Apple account;
6. keep the Mac connected to the Internet while apps and configurations are downloaded.

Initial setup can continue for several minutes after the desktop appears.

If the company profile is not activated during the first startup, activate it
afterwards from **System Settings > General > Device Management** using your
company email address and your Google Workspace password.
...
1. install WireGuard manually from the Mac App Store;
2. create and configure your personal WireGuard access following Operations/IT instructions;
3. activate the tunnel;
4. verify access to the internal target shared by Operations/IT;
5. never share your VPN credentials/profile or reuse them on unauthorized devices.

WireGuard is installed by the user instead of being deployed automatically with
the company profile, because automatic installation can create conflicts on some
managed Macs.
