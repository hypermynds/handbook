...
Hypermynds uses GitHub with SSH authentication. Password-based access is not allowed.
...
Create a dedicated SSH key for GitHub:
...
* Computers must use hard drive encryption:
    ...
    5. store the recovery key in the Hypermynds Bitwarden vault.
...
Hypermynds uses **Bitwarden** as the standard password manager.

All passwords, API tokens, certificates, recovery keys and secrets must be stored in Bitwarden.  
Storing credentials in browsers, notes, files, Slack or email is not allowed.

Install Bitwarden from Homebrew:
...
the expiration time to 12 months (tokens without expiration are unsecure), leave
...
The GitHub token **must also be saved in Bitwarden** under your Hypermynds vault.
Do not store tokens in notes, Slack or plain text files.
...
Never share passwords, tokens, certificates or private keys on Slack, even in private messages.

Slack is not a secure secrets store.
